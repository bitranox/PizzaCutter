...
function run_pytest() {
  ...
  if ! python3 -m pytest "${project_root_dir}" "$@" --cov=pizzacutter; then
...
function test_commandline_interface_venv() {
  ...
  clr_green "issuing command : $HOME/venv/bin/pizzacutter --version"
  if ! "$HOME/venv/bin/pizzacutter" --version; then
